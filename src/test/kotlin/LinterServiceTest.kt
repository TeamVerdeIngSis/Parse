...
class LinterServiceTest {
    ...
    fun `test lintSnippet returns linting errors correctly`() {
        ...
        Mockito.`when`(restTemplate.getForObject("http://localhost:8080/v1/asset/snippets/$snippetId", String::class.java))
            .thenReturn(code)
        Mockito.`when`(restTemplate.getForObject("http://localhost:8080/v1/asset/linting/$userId", String::class.java))
